...
st.divider()
st.subheader("Add a new treatment")

# Determine current user id (optional)
_uid = None
try:
    _uobj = sb.auth.get_user().user
    if _uobj:
        _uid = _uobj.id
except Exception:
    pass
...
_fish = []
try:
    _res = sb.table("fish").select("id,name").order("name").limit(2000).execute()
    _fish = _res.data or []
except Exception as e:
    st.error(f"Could not load fish list: {e}")

_treats = []
try:
    _res2 = sb.table("treatments").select("id,name,default_dose,default_route").order("name").limit(2000).execute()
    _treats = _res2.data or []
except Exception as e:
    st.error(f"Could not load treatments: {e}")

_rna = []
_plasmids = []
_dyes = []
try:
    _r = sb.table("rna").select("id,name").order("name").limit(2000).execute()
    _rna = _r.data or []
except Exception:
    pass
try:
    _p = sb.table("plasmids").select("id,name").order("name").limit(2000).execute()
    _plasmids = _p.data or []
except Exception:
    pass
try:
    _d = sb.table("dyes").select("id,name").order("name").limit(2000).execute()
    _dyes = _d.data or []
except Exception:
    pass

if not _fish:
    st.info("No fish found. Create a fish first.")
else:
    def _fmt_opt(r, label_key="name"):
        label = r.get(label_key) or ""
        rid = r.get("id")
        short = rid[:8] if isinstance(rid, str) else str(rid)
        return f"{label} — {short}" if label else str(rid)

    with st.form("create_treatment_form", clear_on_submit=False):
        c1, c2 = st.columns([2, 1])
        with c1:
            fish_idx = st.selectbox(
                "Fish",
                options=list(range(len(_fish))),
                format_func=lambda i: _fmt_opt(_fish[i]),
            )
        with c2:
            mode = st.radio("Treatment source", options=["Use existing", "Create new"], horizontal=True)

        selected_treatment_id = None
        new_treatment_payload = None
        default_route = None

        if mode == "Use existing":
            if not _treats:
                st.warning("No treatments exist yet. Switch to 'Create new'.")
            else:
                tr_idx = st.selectbox(
                    "Existing treatment",
                    options=list(range(len(_treats))),
                    format_func=lambda i: _fmt_opt(_treats[i]),
                )
                tr = _treats[tr_idx]
                selected_treatment_id = tr["id"]
                default_route = tr.get("default_route")
                default_dose = tr.get("default_dose")
        else:
            c3, c4 = st.columns([2, 1])
            with c3:
                tr_name = st.text_input("Treatment name*", placeholder="Required")
                tr_desc = st.text_area("Description", height=80)
            with c4:
                tr_default_dose = st.text_input("Default dose", placeholder="e.g., 10 mg/kg")
                tr_default_route = st.text_input("Default route", placeholder="e.g., IP, oral")

            c5, c6, c7 = st.columns(3)
            with c5:
                rna_idx = st.selectbox(
                    "RNA (optional)",
                    options=[-1] + list(range(len(_rna))),
                    format_func=lambda i: "— none —" if i == -1 else _fmt_opt(_rna[i]),
                )
            with c6:
                plasmid_idx = st.selectbox(
                    "Plasmid (optional)",
                    options=[-1] + list(range(len(_plasmids))),
                    format_func=lambda i: "— none —" if i == -1 else _fmt_opt(_plasmids[i]),
                )
            with c7:
                dye_idx = st.selectbox(
                    "Dye (optional)",
                    options=[-1] + list(range(len(_dyes))),
                    format_func=lambda i: "— none —" if i == -1 else _fmt_opt(_dyes[i]),
                )

            new_treatment_payload = {
                "name": tr_name.strip() if tr_name else None,
                "description": tr_desc or None,
                "default_dose": tr_default_dose or None,
                "default_route": tr_default_route or None,
            }
            if rna_idx != -1:      new_treatment_payload["rna_id"] = _rna[rna_idx]["id"]
            if plasmid_idx != -1:  new_treatment_payload["plasmid_id"] = _plasmids[plasmid_idx]["id"]
            if dye_idx != -1:      new_treatment_payload["dye_id"] = _dyes[dye_idx]["id"]
            if _uid:               new_treatment_payload["created_by"] = _uid

        st.subheader("Application details")
        colA, colB, colC = st.columns(3)
        with colA:
            app_date = st.date_input("Applied date")
        with colB:
            app_time = st.time_input("Applied time")
        with colC:
            route_override = st.text_input("Route (optional; overrides default route)", value="")

        colD, colE = st.columns(2)
        with colD:
            amount = st.text_input("Amount (text)", value="")
        with colE:
            units = st.text_input("Units (text)", value="")

        notes = st.text_area("Notes", height=80)

        submit = st.form_submit_button("Add treatment")

        if submit:
            try:
                fish_id = _fish[fish_idx]["id"]
                treatment_id = selected_treatment_id

                if mode == "Create new":
                    if not new_treatment_payload or not new_treatment_payload.get("name"):
                        st.error("Treatment name is required.")
                        st.stop()
                    ins = sb.table("treatments").insert(new_treatment_payload).select("id").single().execute()
                    treatment_id = ins.data["id"]
                    default_route = new_treatment_payload.get("default_route")

                if not treatment_id:
                    st.error("Select an existing treatment or create a new one.")
                    st.stop()

                # Build fish_treatments row
                applied_at_iso = None
                if app_date and app_time:
                    applied_at_iso = f"{app_date.isoformat()}T{app_time.isoformat()}"
                ...
                ft_payload = {
                    ...
                    "units": units or None,
                    "route": (route_override or default_route) or None,
                    "notes": notes or None,
                }
                if _uid:
                    ft_payload["created_by"] = _uid

                sb.table("fish_treatments").insert(ft_payload).execute()
                st.success("Treatment recorded.")

                # Show recent treatments for this fish
                _trmap = {}
                try:
                    _alltr = sb.table("treatments").select("id,name").execute().data or []
                    _trmap = {t["id"]: t.get("name") for t in _alltr}
                except Exception:
                    pass

                recent = sb.table("fish_treatments")\
                           .select("*")\
                           .eq("fish_id", fish_id)\
                ...
                for r in recent:
                    r["treatment_name"] = _trmap.get(r.get("treatment_id"))
                if recent:
                    st.caption("Recent treatments for this fish:")
