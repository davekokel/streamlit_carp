...
with st.form("create_cfg"):
    ...
    with c5:
        st.caption("If RLS blocks inserts and this app is server-side only, add `service_role_key` to Streamlit secrets so writes use the service key.")
...
if do_create:
    if selected.empty:
        ...
        for _, row in selected.iterrows():
            ...
            # NEW: prefer fish_name in the handle so default becomes "tank of <fish_name>"
            handle_core = fish_name or fish_code or (str(fish_uuid)[:8] if fish_uuid else str(fish_id))
            suffix = str(uuid.uuid4())[:8]  # keep uniqueness to avoid tanks_name unique constraint
            ...
            # Build base membership fields (prefer UUIDs)
            membership_base = {}
            ...
            # Insert membership probing time column
            mem_row, used_time_col, err = insert_membership_with_time_probe(membership_base)
            if not mem_row:
                errors.append(f"Insert into fish_tank_memberships failed: {err or 'unknown error'}")
                used_time_col = None  # nothing worked
        ...
        if errors:
            with st.expander("Errors & schema tips", expanded=True):
                for e in errors:
                    st.error(e)
                st.markdown("""
**If you saw `PGRST204` about a column missing in the schema cache:**
- It usually means that column doesn’t exist **or** PostgREST hasn’t reloaded the schema yet.
- You can reload the schema, or we can simply omit that column (this page already retries without it).

**Reload PostgREST schema (terminal):**
```bash
...
**Check columns of the join table (terminal):**
```bash
cat <<'SQL' | psql "$STAGING_DB_URL" -v ON_ERROR_STOP=1
