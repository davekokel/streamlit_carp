...
if not _fish:
    ...
    with st.form("create_treatment_form", clear_on_submit=False):
        ...
        if mode == "Use existing":
            ...
            with c5:
                rna_opt = [None] + _rna
                rna_sel = st.selectbox(
                    "RNA (optional)",
                    options=rna_opt,
                    format_func=lambda r: "— none —" if r is None else _fmt_opt(r),
                )
            with c6:
                plasmid_opt = [None] + _plasmids
                plasmid_sel = st.selectbox(
                    "Plasmid (optional)",
                    options=plasmid_opt,
                    format_func=lambda r: "— none —" if r is None else _fmt_opt(r),
                )
            ...
            if rna_sel is not None:      new_treatment_payload["rna_id"] = rna_sel["id"]
            if plasmid_sel is not None:  new_treatment_payload["plasmid_id"] = plasmid_sel["id"]
            if dye_sel is not None:      new_treatment_payload["dye_id"] = dye_sel["id"]
