...
# --- end v30 helpers ---
    ...
    def _link_trt_plasmids(tid: str, plasmid_ids: list[str]):
        if not plasmid_ids:
            return
        rows = [{"treatment_id": tid, "plasmid_id": st.session_state.get("_plasmid_uuid_to_id_map", {}).get(str(pid))} for pid in plasmid_ids]
        try:
            rows = [r for r in rows if r.get("plasmid_id") is not None]
            try:
