...
def insert_returning(table: str, obj: dict) -> Optional[dict]:
    try:
        # select('*') on insert returns the inserted row (PostgREST feature)
        res = get_sb().table(table).insert(obj).select("*").execute()
        rows = res.data or []
...
if do_create:
    if selected.empty:
        ...
        # Build a quick lookup from fish code/name to keep names human and unique
        # We will include a short uuid suffix to avoid unique name collisions
        for _, row in selected.iterrows():
            ...
            new_tank = insert_returning("tanks", tank_insert)
            if not new_tank:
                continue
            ...
            # membership link (prefer UUIDs if present)
            membership = {
