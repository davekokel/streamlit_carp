...
def build_linked_feature_columns(fish_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["tanks_list"] = df[key_col].astype(str).map(lambda k: list(meta.get(k, {}).get("labels", [])))
    df["tanks"]      = df["tanks_list"].apply(lambda xs: ", ".join(xs))
    df["tank_sites"] = df[key_col].astype(str).map(lambda k: list(meta.get(k, {}).get("sites", [])))
    return df

@st.cache_data(ttl=60, show_spinner=False)
def fetch_tank_meta_bulk(fish_ids: list[str]) -> dict[str, dict]:
    if not fish_ids:
        return {}
    fish_ids = [str(x) for x in fish_ids]
    try:
        mem = sb.table("fish_tank_memberships").select("fish_id_uuid,tank_id_uuid,valid_to").in_("fish_id_uuid", fish_ids).execute().data or []
    except Exception:
        mem = []
    active = []
    if mem:
        ...
        for r in mem:
            ...
            if ok:
                active.append(r)
    tank_ids = sorted({str(r.get("tank_id_uuid")) for r in active if r.get("tank_id_uuid")})
    tmap = {}
    if tank_ids:
        try:
            trows = sb.table("tanks").select("id_uuid,tank_code,code,name,site_code").in_("id_uuid", tank_ids).execute().data or []
        except Exception:
            trows = []
        def _lab(row: dict) -> str:
            for k in ("tank_code","code","name"):
                v = row.get(k)
                if v:
                    return str(v)
            return str(row.get("id_uuid") or "")
        tmap = {str(r.get("id_uuid")): {"label": _lab(r), "site": r.get("site_code")} for r in trows}
    out: dict[str, dict] = {str(fid): {"labels": [], "sites": []} for fid in fish_ids}
    for r in active:
        fid = str(r.get("fish_id_uuid"))
        tid = str(r.get("tank_id_uuid"))
        if not fid or not tid:
            continue
        m = tmap.get(tid, {"label": tid, "site": None})
        out.setdefault(fid, {"labels": [], "sites": []})
        out[fid]["labels"].append(m["label"])
        if m["site"]:
            out[fid]["sites"].append(str(m["site"]))
    ...
    df["mutations"]  = df[key_col].astype(str).map(lambda k: ", ".join(mu.get(k, [])))
    df["treatments"] = df[key_col].astype(str).map(lambda k: ", ".join(tr.get(k, [])))
    df["tanks"]      = df[key_col].astype(str).map(lambda k: ", ".join(tk.get(k, [])))
    return df

@st.cache_data(ttl=60, show_spinner=False)
def fetch_tank_labels_bulk(fish_ids: list[str]) -> dict[str, list[str]]:
    if not fish_ids:
        return {}
    fish_ids = [str(x) for x in fish_ids]
    try:
        ...
        mem = []
    # Filter active (valid_to null or future)
    active = []
    if mem:
        from datetime import datetime, timezone
        now = datetime.now(timezone.utc)
    ...
    tank_ids = sorted({str(r.get("tank_id_uuid")) for r in active if r.get("tank_id_uuid")})
    if not tank_ids:
        return {str(fid): [] for fid in fish_ids}
    try:
        trows = sb.table("tanks").select("id_uuid,name,tank_code,code").in_("id_uuid", tank_ids).execute().data or []
    except Exception:
        trows = []
    def _lab(row: dict) -> str:
        for k in ("tank_code","code","name"):
            ...
            if v:
                return str(v)
        return str(row.get("id_uuid") or "")
    tmap = {str(r.get("id_uuid")): _lab(r) for r in trows}
    out: dict[str, list[str]] = {str(fid): [] for fid in fish_ids}
    for r in active:
        fid = str(r.get("fish_id_uuid"))
        tid = str(r.get("tank_id_uuid"))
        if fid and tid:
            out.setdefault(fid, []).append(tmap.get(tid, tid))
    # de-dup
    for k, v in out.items():
        seen = set(); dedup = []
        for x in v:
...
def _build_tank_payloads(labels: list[str], uid: str, site: str | None = None) -> list[dict]:
    ...
    for lab in labels:
        ...
        if "site_code" in (info.get("keys") or set()) and site:
            row["site_code"] = str(site)
...
def _filter_local(df: pd.DataFrame, term: str) -> pd.DataFrame:
    ...
    if not term or df.empty:
        return df
    cols = [c for c in ["code","fish_code","name","line_building_stage","notes","transgenes","mutations","treatments","tanks"] if c in df.columns]
...
_site_opts = sorted({s for xs in fish_df.get('tank_sites', []) for s in (xs if isinstance(xs, list) else [])}) if 'tank_sites' in fish_df.columns else []
_opts = ['(all)'] + _site_opts
_default = st.session_state.get('site_filter', '(all)')
try:
    _idx = _opts.index(_default)
except ValueError:
    _idx = 0
_site_sel = st.selectbox('Filter by site', _opts, index=_idx, key='site_filter')
if _site_sel != '(all)' and 'tank_sites' in fish_df.columns:
    fish_df = fish_df[fish_df['tank_sites'].apply(lambda xs: isinstance(xs, list) and _site_sel in xs)]
...
front_cols = [c for c in ["code","fish_code","name","date_birth","stage","sex","transgenes","mutations","treatments","tanks_list"] if c in fish_df.columns]
...
cfg = {}
_all_tanks = sorted({t for row in (disp["tanks_list"] .tolist() if "tanks_list" in disp.columns else []) for t in (row if isinstance(row, list) else [])})
if "tanks_list" in disp.columns:
    cfg["tanks_list"] = st.column_config.MultiSelectColumn("Tanks", options=_all_tanks, width="medium")
...
with st.expander("Create new tanks (optional)"):
    ...
    _new_tanks_site = st.selectbox("Site", ["NURSERY","ADULT"], index=0, key="_new_tanks_site")
    ...
    if _create_tanks:
        ...
        if not uid:
            ...
            if not labels:
                ...
                try:
                    payloads = _build_tank_payloads(labels, uid=str(uid), site=_new_tanks_site)
